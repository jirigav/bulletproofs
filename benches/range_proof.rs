...
mod bench_range_proof {
    ...
    use curv::elliptic::curves::{Generator, Point, Scalar, Secp256r1};
    use curv::BigInt;
    use sha2::Sha512;
            ...
            move |b, &&m| {
                ...
                let v_vec = (0..m)
                    .map(|_i| {
                        ...
                        Scalar::<Secp256r1>::from(&v)
                    })
                    .collect::<Vec<Scalar<Secp256r1>>>();

                let r_vec = (0..m)
                    .map(|_i| Scalar::<Secp256r1>::random())
                    .collect::<Vec<Scalar<Secp256r1>>>();
    ...
    pub fn bench_range_proof_16_2(c: &mut Criterion) {
        c.bench_function("range proof, n=16, m=2", move |b| {
            ...
            let g: Generator<Secp256r1> = Point::generator();
            let label = BigInt::from(1);
            let hash = Sha512::new().chain_bigint(&label).result_bigint();
            let h = generate_random_point(&Converter::to_bytes(&hash));
...
mod bench_wip_range_proof {
    ...
    use criterion::Criterion;
    use curv::arithmetic::traits::*;
    use curv::elliptic::curves::{Point, Scalar, Secp256r1};
    use curv::BigInt;
